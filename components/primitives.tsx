...
// ──────────────────────────────────────────────────────────────────────
// NavGauge — diverging premium/discount meter for mNAV. Par (1.0x = a
// share priced exactly at the ZEC it's backed by) sits dead-center; the
// bar fills from par toward the live marker, tinted green for a premium
// (>1x) and red for a discount (<1x). It answers the one question a
// crypto-treasury tracker exists to answer — "is the market pricing this
// above or below its coins?" — in a single glanceable row, so the tile
// no longer needs four stacked discount readouts to say it.
//
// The window is a symmetric 0.5x–1.5x band around par so the normal
// treasury-stock range is legible; values outside clamp to the track edge
// (the headline number carries the exact figure). `scale` adds tiny end
// labels for detail surfaces (treasury page) that have the room.
// ──────────────────────────────────────────────────────────────────────
export function NavGauge({
  value,
  height = 8,
  scale = false,
}: {
  ...
  const premium = ok && value >= 1
  const accent = !ok
    ? paletteVar("text")
    : premium
      ? paletteVar("cyph")
      : E_STATIC.red
  const track = paletteVar("ratio")
  const markerPct = ok
    ...
    <div className="w-full">
      <div
        ...
        style={{
          height,
          background: `${track}12`,
          border: `1px solid ${track}2b`,
        }}
        role="img"
        aria-label={
          ok
            ? `mNAV ${value.toFixed(2)}x — ${premium ? "premium to" : "discount to"} net asset value; par is 1.0x`
            : "mNAV unavailable"
        }
      >
        {/* Fill spans from par (center) to the live marker. */}
        {ok && fillWidth > 0 && (
          <div
        ...
        {/* Par tick — the reference line at 1.0x. */}
        <div
          className="absolute top-0 bottom-0"
          style={{
            left: "50%",
      ...
      {scale && (
        <div
          className="mt-1 flex justify-between text-[8px] tabular-nums tracking-[0.1em]"
